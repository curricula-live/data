...
def content_hash(rows):
    return hashlib.sha256(serialize_jsonl(rows).encode("utf-8")).hexdigest()


def validate_unicode(value, location="record"):
    if isinstance(value, str):
        if unicodedata.normalize("NFC", value) != value:
            raise SystemExit(f"{location}: strings must use NFC Unicode normalization")
        if any(0xD800 <= ord(character) <= 0xDFFF for character in value):
            raise SystemExit(f"{location}: invalid Unicode surrogate")
...
def validate_rows(rows, schema_path, data_path):
    ...
    for line_number, row in enumerate(rows, 1):
        validate_unicode(row, f"{data_path}:{line_number}")
        ...
        json.dumps(
            {
                ...
                "concept_sha256": content_hash(concepts),
                "full_snapshot": True,
                "relation_count": len(relations),
                "relation_sha256": content_hash(relations),
                "source": "supabase",
            },
            sort_keys=True,
            separators=(",", ":"),
...
def push(apply=False, prune=False, confirm_prune=""):
    ...
    if prune:
        verify_full_snapshot(repo_concepts, repo_relations)
        ...
        for row in repo_concepts:
            cursor.execute(
                ...
                on conflict (slug) do update set
                    ...
                    updated_at = now()
                where (concept.label, concept.description, concept.metadata)
                    is distinct from
                    (excluded.label, excluded.description, excluded.metadata)
                """,
                (
                    row["slug"],
                    row["label"],
        ...
        for row in repo_relations:
            cursor.execute(
                ...
                on conflict (source, target, type) do update set
                    metadata = excluded.metadata,
                    updated_at = now()
                where relation.metadata is distinct from excluded.metadata
                """,
                (
                    row["type"],
                    Jsonb(row["metadata"]),
...
def verify_full_snapshot(concepts, relations):
    try:
        marker = read_json(SNAPSHOT)
    except (OSError, json.JSONDecodeError) as exc:
        raise SystemExit("Pruning requires a valid full-snapshot marker generated by pull") from exc
    expected = {
        "full_snapshot": True,
        "source": "supabase",
        "concept_count": len(concepts),
    ...
    if any(marker.get(key) != value for key, value in expected.items()):
        raise SystemExit("Pruning refused: snapshot marker does not match repository data")
